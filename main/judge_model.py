...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, nclass, 
                 dropout_em=0.5,dropout_rnn=0,dropout_out=0, tie_weights=False, n_cl_hidden=30, beta_size=300,device='cpu'):
        ...
        self.beta = nn.Parameter(torch.rand((1, beta_size, 1))*(2/beta_size))
    ...
    def forward(self, input, hidden, last_location, g_label):
        ...
        decoded = self.judge1_out(output)
        pad = torch.zeros((self.beta_size-decoded.size()[0], decoded.size()[1], decoded.size()[2])).to(self.device)
        #print(pad)
        decoded = torch.cat([decoded, pad])
        decoded = torch.bmm(decoded.permute(1, 2, 0), self.beta.expand(decoded.size()[1], -1, -1))
        D_flabel = torch.cat((decoded.squeeze(2), g_label), 1)
