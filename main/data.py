...
class TEC_ISEAR_DataSet(Dataset):
    ...
    def load(self, dictionary=None):
        if self.file == 'isear':
            ...
            #data = loader.load_tec('RLANS/data_collection/tec.txt')
            data = TecDataset([],[])
            data.text_data = json.load(open('tec_data_da.json'))
            data.target = json.load(open('tec_target_da.json'))

        train_dataset = TEC_ISEAR_DataSet(self.file)
        test_dataset = TEC_ISEAR_DataSet(self.file)
        ...
        for label, text in zip(data.get_target(), data.get_data()):
            ...
            if type(label) == list:
                label = label[0]
            if train_mode:
                train_dataset.labels.append(label-1)
                train_dataset.tokens.append(token)
            else:
                test_dataset.labels.append(label-1)
                test_dataset.tokens.append(token)

            if len(token) > self.max_length: self.max_length = len(token)
        train_dataset.length = len(train_dataset.labels)
        train_dataset.max_length = self.max_length
        test_dataset.length = len(test_dataset.labels)
