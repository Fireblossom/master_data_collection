...
import string
remove_list = set(string.punctuation)
    ...
    def __cleaning(self, text_list):
        ...
        for text in text_list:
            ...
            if self.tokenize:
                ...
                if self.level != 0:
                    vocab = pickle.load(open('data_collection/py_isear/vocab'+str(self.level)+'.pkl', 'rb'))
...
class IsearLoader:
    ...
    def load_isear(self, s_isear_path, level, data = False):
        ...
        f_isear.close()
        if data:
            addition = IsearDataSet(attributes_subset,
                                target_subset,
                                text_data,
                                self.tokenize,
                                level)
            data.set_data(data.get_data() + addition.get_data())
            target = [-1] * len(text_data)
            data.set_target(data.get_target() + target)
        else:
