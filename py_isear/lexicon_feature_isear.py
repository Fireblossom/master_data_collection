...
documents = []
for text, target in zip(dataset.get_freetext_content(), dataset.get_target()):
    documents.append((text, target[0]))
random.shuffle(documents)

all_tokens = [element for lis in dataset.get_freetext_content() for element in lis]
...
features = classifier.most_informative_features(n=20)
...
import json
json.dump(features_count, open('isear_feature_count.json', 'w'), indent=2)