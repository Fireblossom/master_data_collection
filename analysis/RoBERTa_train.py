...
class TweetDataset(torch.utils.data.Dataset):
    def __init__(self, df, max_len=200):
        ...
        with open('roberta-base/merges.txt', encoding="utf-8") as merges_handle:
            bpe_merges = merges_handle.read().split("\n")[1:-1]
        bpe_merges = [tuple(merge.split()) for merge in bpe_merges]

        self.tokenizer = tokenizers.ByteLevelBPETokenizer(
            vocab=json.load(open('roberta-base/vocab.json', encoding="utf-8")), 
            merges=bpe_merges, 
            lowercase=True,
...
class TweetModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        hs = self.roberta(input_ids, attention_mask)[2]
        # print(type(hs))
...
def dist_between(start_logits, end_logits, device='cpu', max_seq_len=96):
...
def dist_loss(start_logits, end_logits, start_positions, end_positions, device=torch.device('cuda'), max_seq_len=96, scale=1):
...
def loss_fn(start_logits, end_logits, start_positions, end_positions):
    ...
    distan_loss = dist_loss(start_logits, end_logits, start_positions, end_positions)
    return idx_loss+distan_loss
...
if __name__ == '__main__':
    ...
    sub_df = pd.read_csv('tweet-sentiment-extraction/sample_submission.csv')
